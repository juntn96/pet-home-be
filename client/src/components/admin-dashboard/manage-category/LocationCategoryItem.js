...
class LocationCategoryItem extends Component {
  ...
  _deleteItem = () => {
    this.props.onDeleteHandle({id: this.props.locationCate._id,deletionFlag: this.props.locationCate.hiddenFlag,name: this.props.locationCate.name}); 
  }
  ...
  render() {
    ...
    <Card className={typeColor} >
        <CardHeader>
          {type}<small className="text-muted">  {date}</small>
        ...
        <div className="callout callout-danger">
          ...
          <strong className="h4 nameItemValue">{locationCate.name}</strong>
          </div>
        </div>
        </CardBody>
      </Card>
